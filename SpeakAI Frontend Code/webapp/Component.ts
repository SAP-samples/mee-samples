...
export default class Component extends UIComponent {
	...
	public init(): void {
		...
		// UI model for dynamic date/time in subheader
        const uiModel = new JSONModel({ dateLabel: "", timeLabel: "", showScrollToBottom: false, sidebarOpen: false });
        const updateNow = () => {
            const d = new Date();
            uiModel.setProperty("/dateLabel", d.toLocaleDateString(undefined, {
                weekday: "long", month: "long", day: "numeric", year: "numeric"
            }));
            uiModel.setProperty("/timeLabel", d.toLocaleTimeString(undefined, {
	...
	private _injectInitialGreeting(agentModel: JSONModel): void {
		try {
			...
			if (rbOrPromise && typeof rbOrPromise.then === "function") {
				(rbOrPromise as Promise<any>).then((rb) => {
