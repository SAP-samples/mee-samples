...
export default class Main extends BaseController {
    ...
    public onInit(): void {
        ...
        const log = (...a: any[]) => { if (semanticDebug) { // eslint-disable-next-line no-console
            console.log("[SemanticAttach]", ...a); } };
        const warn = (...a: any[]) => { if (semanticDebug) { // eslint-disable-next-line no-console
            ...
            try {
                ...
                if (!table) {
                    if (attempts < maxAttempts) {
                        if (attempts % 5 === 0) { log("Waiting for table... attempt", attempts); }
    ...
    public onToggleSidebar(oEvent: any): void {
        const pressed = typeof oEvent?.getParameter === "function"
    ...
    private _ensureScrollBinding(): void {
        ...
        this._boundResize = () => this._updateScrollToBottomVisibility();
    ...
    private _registerChatResize(): void {
        ...
        if (!this._boundResize) {
            this._boundResize = () => this._adjustChatLayout();
            window.addEventListener("resize", this._boundResize);
            window.addEventListener("orientationchange", this._boundResize as any);
        }
    }
    ...
    private _deregisterChatResize(): void {
        if (this._resizeRegId) {
            try { ResizeHandler.deregister(this._resizeRegId); } catch { /* ignore */ }
